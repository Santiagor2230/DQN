...
class DeepQlearning(LightningModule):
  ...
  def __init__(self, env_name, policy=epsilon_greedy, capacity=100_000,
               ...
               sync_rate=10):
    super().__init__()
    self.env = environment.create_envrionment(env_name)
  ...
  def training_step(self, batch, batch_idx):
    ...
    next_action_values, _ = self.target_q_net(next_states).max(dim=1, keepdim=True) #gives the highest reward with correspoding action
...
model = DeepQlearning("LunarLander-v2")
